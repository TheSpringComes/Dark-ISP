...
model = dict(
    ...
    roi_head=dict(
        ...
        bbox_head=[
            dict(
                type='DIIHead',
                num_classes=3,  # 8 classes in LOD
                num_ffn_fcs=2,
                num_heads=8,
                num_cls_fcs=1,
                num_reg_fcs=3,
        ...
        dataset=dict(
            ...
            datasets=[
                dict(
                    type=dataset_type,
                    data_root=data_root,
                    ann_file='annotations/Sony/rgb_new_Sony_RX100m7_train.json',
                    data_prefix=dict(img='Sony_RGB/'),
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='annotations/Sony/rgb_new_Sony_RX100m7_val.json',
        data_prefix=dict(img='Sony_RGB/'),
        test_mode=True,
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'annotations/Sony/rgb_new_Sony_RX100m7_val.json',
    metric='bbox',
    format_only=False,
    backend_args=backend_args)
...
test_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'annotations/Sony/rgb_new_Sony_RX100m7_test.json',
        data_prefix=dict(img='Sony_RGB/'),
        test_mode=True,
        pipeline=test_pipeline))
test_evaluator = dict(
...
#     dict(
#         type='MultiStepLR',
#         milestones=[5, 10],  # 在第5和第10个周期调整学习率
