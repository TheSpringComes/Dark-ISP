...
class UNetArch(nn.Module):
    def __init__(self, inchannels=3, outchannels=3, channels=32, load_path=None, fozen=True) -> None:
        super().__init__()
    ...
    def loading_repnr_checkpoint(self):
        if self.load_path is not None:
            self.load_network(self.encoder, self.load_path)
        if self.load_path is not None:
            self.load_network(self.decoder, self.load_path)
