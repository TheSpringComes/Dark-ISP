...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def _get_pad_shape(self, data: dict) -> List[tuple]:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            batch_pad_shape = []
            for i in range(len(_batch_inputs)):
                ori_input = _batch_inputs[i]
                pad_h = int(
...
@MODELS.register_module()
class DetRAWDataPreprocessor(ImgDataPreprocessor):
    def __init__(self,
                 mean: Sequence[Number] = None,
                 std: Sequence[Number] = None,
                 pad_size_divisor: int = 1,
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        data = super().forward(data=data, training=training)
        if 'gts' in data.keys():
            inputs, gts, data_samples = data['inputs'], data['gts'], data['data_samples']
        else:
            inputs, data_samples = data['inputs'], data['data_samples']
            gts = None
        ...
        return {'inputs': inputs, 'gts': gts, 'data_samples': data_samples}
